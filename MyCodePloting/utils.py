...
from MyCodeTools.utils import strings_to_categoricals
...
def default_basis(adata, **kwargs):
    ...
    if "x" in kwargs and "y" in kwargs:
        ...
        if "acc_embedding" in adata.obsm.keys():
            del adata.obsm["acc_embedding"]
    else:
        keys = [
        ...
        if is_color_like(c) and c not in adata.var_names:
            ...
            if layer in adata.layers.keys():
                if perc is None and any(
                    layer_name in layer
                    for layer_name in ["spliced", "unspliced", "Ms", "Mu", "acc"]
                ):
...
def plot_vlines(adata, basis, vkey, xkey, linewidth=1, linecolor=None, ax=None):
    ...
    fits = [
        ...
        if all(["acc" in fit, f"{fit}_gamma" in adata.var.keys()])
    ]
...
def plot_acc_fits(
    ...
    # linear fits
    if vkey is None:
        vkey = "dynamics" if "fit_alpha" in adata.var.keys() else "acc"
...
# TODO: Add docstrings
def acc_embedding_changed(adata, basis, vkey):
    """TODO."""
...
def get_temporal_connectivities(adata, tkey, n_convolve=30):
    ...
    from scvelo.tools.acc_graph import vals_to_csr
