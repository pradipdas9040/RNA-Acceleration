...
from MyCodeTools._em_model_core import latent_time
from MyCodeTools.rank_acc_genes import rank_acc_genes
from MyCodeTools.score_genes_cell_cycle import score_genes_cell_cycle
from MyCodeTools.acc_pseudotime import acc_pseudotime
...
def summary(adata, basis="umap", color="clusters", n_top_genes=12, var_names=None):
    ...
    if "fit_alpha" in adata.var.keys():
        ...
        tkey = "acc_pseudotime"
        acc_pseudotime(adata)
        if "rank_acc_genes" not in adata.uns.keys():
            rank_acc_genes(adata, groupby=color)
        top_genes = pd.DataFrame(adata.uns["rank_acc_genes"]["names"])
    ...
    with GridSpec(ncols=4, nrows=nrows) as pl:
        ...
        pl.acc_embedding_stream(adata, c=tkey)
