...
from MyCodeTools.acc_embedding import (
    acc_embedding as compute_acc_embedding,
)
...
def acc_embedding(
    ...
    **kwargs,
    ...
    if V is None:
        for key in vkeys:
            for bas in bases:
                if recompute or acc_embedding_changed(adata, basis=bas, vkey=key):
                    compute_acc_embedding(adata, basis=bas, vkey=key)
