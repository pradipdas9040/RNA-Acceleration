...
from MyCodeTools.paga import get_igraph_from_adjacency
...
def paga(
    ...
    **kwargs,
    ...
    """Plot PAGA graph with acc-directed edges.
    ...
    if cb_kwds is None:
        cb_kwds = {}
    if vkey == "all":
        vkey = [k for k in adata.layers.keys() if "acc" in k and "_u" not in k]
    ...
    if multikey is not None:
        ...
        if title is None:
            title = (
                ...
                else f"paga acc-graph ({paga_groups})"
            )
