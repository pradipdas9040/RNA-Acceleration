    ...
    if multikey is not None:
        ...
        # actual scatter plot
            ...
            if basis in adata.var_names:
                ...
                # acc model fits (full dynamics and steady-state ratios)
            ...
            # set vmid to 0 if color values obtained from acc expression
            if not np.any([v in kwargs for v in ["vmin", "vmid", "vmax"]]) and np.any(
                [
                    isinstance(v, str)
                    and "time" not in v
                    and (v.endswith("acc") or v.endswith("transition"))
