...
from MyCodeTools.utils import groups_to_bool
from MyCodeTools.acc_embedding import acc_embedding
...
from .utils import (
    ...
    make_unique_list,
    savefig_or_show,
    acc_embedding_changed,
)
from .acc_embedding_grid import compute_acc_on_grid


@doc_params(scatter=doc_scatter)
def acc_embedding_stream(
    adata,
    basis=None,
    vkey="acc",
    density=2,
    ...
    **kwargs,
    ...
    V: `np.ndarray` (default: None)
        Embedding acceleration coordinates. Using `adata.obsm['acc_umap']` per default.
    ...
    if vkey == "all":
        ...
        vkey = [key for key in lkeys if "acc" in key and "_u" not in key]
    ...
    if V is None:
        for key in vkeys:
            if recompute or acc_embedding_changed(adata, basis=basis, vkey=key):
                acc_embedding(adata, basis=basis, vkey=key)
    ...
    if X_grid is None or V_grid is None:
        ...
        X_grid, V_grid = compute_acc_on_grid(
            X_emb=X_emb,
            V_emb=V_emb,
            density=1,
            smooth=smooth,
