...
from MyCodeTools.rank_acc_genes import rank_acc_genes
...
def acc(
    ...
    **kwargs,
    ...
    """Phase and acc plot for set of genes.

    The phase plot shows spliced against unspliced expressions with steady-state fit.
    Further the embedding is shown colored by acc and expression.
    ...
    fits: `str` or list of `str` (default: `['acc', 'dynamics']`)
    ...
    color_map: `str` or tuple (default: `['RdYlGn', 'gnuplot_r']`)
        ...
        color map correspond to acc and expression, respectively.
    ...
    if isinstance(groupby, str) and groupby in adata.obs.keys():
        if (
            "rank_acc_genes" not in adata.uns.keys()
            or adata.uns["rank_acc_genes"]["params"]["groupby"] != groupby
        ):
            rank_acc_genes(adata, vkey=vkey, n_genes=10, groupby=groupby)
        names = np.array(adata.uns["rank_acc_genes"]["names"].tolist())
        if groups is None:
            var_names = names[:, 0]
        else:
