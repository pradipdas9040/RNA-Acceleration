...
def transition_matrix(
    ...
    from the acc graph :math:`\pi_{ij}`, with row-normalization :math:`z_i` and
    ...
    vgraph: csr matrix or `None` (default: `None`)
        acc graph representation to use instead of adata.uns[f'{vkey}_graph'].
    ...
    if f"{vkey}_graph" not in adata.uns:
        raise ValueError(
            "You need to run `tl.acc_graph` first to compute cosine correlations."
        )
    ...
    if f"X_{basis}" in adata.obsm.keys():
        ...
        T = T.multiply(diffusion_kernel)  # combine acc kernel & diffusion kernel
