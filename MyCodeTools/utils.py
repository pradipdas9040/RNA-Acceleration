...
# TODO: Add docstrings
def randomized_acc(adata, vkey="acc", add_key="acc_random"):
    """TODO."""
    ...
    acc_graph(adata, vkey=add_key)
    acc_embedding(adata, vkey=add_key, autoscale=False)
...
def cutoff_small_velocities(
    adata, vkey="acc", key_added="acc_cut", frac_of_max=0.5, use_raw=False
):
    """TODO."""
    ...
    acc_graph(adata, vkey=key_added, approx=True)
    acc_embedding(adata, vkey=key_added)
...
# TODO: Finish docstrings
def get_extrapolated_state(adata, vkey="acc", dt=1, use_raw=None, dropna=True):
