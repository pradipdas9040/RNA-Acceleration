...
class VELOVI(VAEMixin, UnsupervisedTrainingMixin, BaseModelClass):
    ...
    **model_kwargs
    ...
    def get_acc(
        ...
        """Returns cells by genes acc estimates.
        ...
        return_numpy
            ...
            Otherwise, it defaults to `True`.
        velo_statistic
            Whether to compute expected acc over states, or maximum a posteriori acc over maximal
            probability state.
        velo_mode
...
def _directional_statistics_per_cell(
    ...
    # over samples axis
    mean_acc_of_cell = tensor.mean(0)
    cosine_sims = [
        _cosine_sim(tensor[i, :], mean_acc_of_cell) for i in range(n_samples)
    ]
