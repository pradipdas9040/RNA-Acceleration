...
class acc:
    ...
    def __init__(
        ...
        self._acc_genes = np.ones(n_vars, dtype=bool)
    ...
    def compute_deterministic(self, fit_offset=False, perc=None):
        ...
        if self._highly_variable is not None:
            self._acc_genes &= self._highly_variable

        if np.sum(self._acc_genes) < 2:
            min_r2 = np.percentile(self._r2, 80)
            self._acc_genes = self._r2 > min_r2
    ...
    def compute_stochastic(
        ...
        idx = np.ones(self._acc_genes.shape, dtype=bool)
        if np.any(self._acc_genes):
            idx = self._acc_genes
...
def acc(
    ...
    **kwargs,
    ...
    generalizes RNA acc estimation to transient systems. It is also capable of
    ...
    vkey: `str` (default: `'acc'`)
        ...
        for `acc_graph` and `acc_embedding`.
    ...
    filter_genes: `bool` (default: `True`)
        Whether to remove genes that are not used for further acc analysis.
    groups: `str`, `list` (default: `None`)
        Subset of groups, e.g. [‘g1’, ‘g2’, ‘g3’],
        to which acc analysis shall be restricted.
    ...
    use_latent_time: `bool`or `None` (default: `None`)
        Whether to use latent time as a regularization for acc estimation.
    ...
    min_likelihood: `float` (default: `None`)
        Minimal likelihood for acc genes to fit the model on.
    ...
    -------
    acc: `.layers`
        acc vectors for each individual cell
    acc_genes, acc_beta, acc_gamma, acc_r2: `.var`
        parameters
    """
    if perc is None:
        perc = [5, 95]
    ...
    logg.info("computing accelerations", r=True)
    ...
    if mode in {"dynamical", "dynamical_residuals"}:
        ...
        if "residuals" in mode:
            ...
            vt, wt = get_divergence(vdata, mode="acc", **kwargs_)
        ...
        for cat in categories:
            ...
            if mode == "stochastic":
                if filter_genes and len(set(velo._acc_genes)) > 1:
                    adata._inplace_subset_var(velo._acc_genes)
                    residual = velo._residual[:, velo._acc_genes]
                    _adata = adata if groups is None else adata[cell_subset]
                    velo = acc(
                        _adata,
                        residual=residual,
    ...
    if f"{vkey}_genes" in adata.var.keys() and np.sum(adata.var[f"{vkey}_genes"]) < 10:
        logg.warn(
            "Too few genes are selected as acc genes. "
    ...
    logg.hint(
        "added \n"
        f"    '{vkey}', acc vectors for each individual cell (adata.layers)"
    )
...
def acc_genes(
    ...
    acc_genes: `.var`
        genes to be used for further acc analysis (acc graph and embedding)
    """
    adata = data.copy() if copy else data
    ...
    logg.info("Number of obtained acc_genes:", np.sum(adata.var[f"{vkey}_genes"]))
