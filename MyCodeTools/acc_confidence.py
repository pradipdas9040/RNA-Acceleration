...
def acc_confidence(data, vkey="acc", copy=False):
    """Computes confidences of velocities.

    .. code:: python

        scv.tl.acc_confidence(adata)
        scv.pl.scatter(adata, color="acc_confidence", perc=[2, 98])
    ...
    -------
    acc_length: `.obs`
        Length of the acc vectors for each individual cell
    acc_confidence: `.obs`
    ...
    if vkey not in adata.layers.keys():
        raise ValueError("You need to run `tl.acc` first.")
    ...
    if f"{vkey}_confidence_transition" not in adata.obs.keys():
        acc_confidence_transition(adata, vkey)

    return adata if copy else None


def acc_confidence_transition(data, vkey="acc", scale=10, copy=False):
    """Computes confidences of acc transitions.
...
def score_robustness(
    data, adata_subset=None, fraction=0.5, vkey="acc", copy=False
):
    """TODO."""
    adata = data.copy() if copy else data
