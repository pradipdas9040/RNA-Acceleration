...
from .acc_pseudotime import acc_pseudotime
...
def acc_clusters(
    ...
    vkey="acc",
    match_with="clusters",
    sort_by="acc_pseudotime",
    resolution=None,
    min_likelihood=None,
    copy=False,
):
    """Computes acc clusters via louvain on velocities.

    .. code:: python

        scv.tl.acc_clusters(adata)
        scv.pl.scatter(adata, color="acc_clusters")
    ...
    -------
    acc_clusters : `.obs`
        Clusters obtained from applying louvain modularity on acc expression.
    """
    adata = data.copy() if copy else data

    logg.info("computing acc clusters", r=True)
    ...
    if sort_by == "acc_pseudotime" and sort_by not in adata.obs.keys():
        acc_pseudotime(adata, vkey=vkey)
    ...
    logg.hint(
        "added \n"
        f"    '{vkey}_clusters', "
        f"clusters based on louvain modularity on acc vector field (adata.obs)"
    )
...
def rank_acc_genes(
    ...
    """Rank genes for acc characterizing groups.

    This applies a differential expression test (Welch t-test with overestimated
    variance to be conservative) on acc expression, to find genes in a cluster that
    ...
    If no clusters are given, it priorly computes acc clusters by applying louvain
    modularity on acc expression.

    .. code:: python

        scv.tl.rank_acc_genes(adata, groupby="clusters")
        scv.pl.scatter(adata, basis=adata.uns["rank_acc_genes"]["names"]["Beta"][:3])
        pd.DataFrame(adata.uns["rank_acc_genes"]["names"]).head()
    ...
    if groupby is None or groupby == "acc_clusters":
        acc_clusters(
            adata,
            vkey=vkey,
    ...
    logg.info("ranking acc genes", r=True)
