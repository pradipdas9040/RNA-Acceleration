...
def recover_dynamics(
    ...
    **kwargs,
    ...
    var_names: `str`,  list of `str` (default: `'acc_genes'`)
        Names of variables/genes to use for the fitting. If `var_names='acc_genes'`
        but there is no column `'acc_genes'` in `adata.var`, acc genes are
        estimated using the steady state model.
    n_top_genes: `int` or `None` (default: `None`)
        Number of top acc genes to use for the dynamical model.
    ...
    if isinstance(var_names, str) and var_names not in adata.var_names:
        if var_names in adata.var.keys():
            ...
            velo = acc(adata, use_raw=use_raw)
            velo.compute_deterministic(perc=[5, 95])
            var_names = adata.var_names[velo._acc_genes]
...
def latent_time(
    ...
    min_corr_diffusion: `float` between `0` and `1` or `None` (default: `None`)
        Only select genes that correlate with acc pseudotime obtained
        from diffusion random walk on acc graph.
    weight_diffusion: `float` or `None` (default: `None`)
        Weight applied to couple latent time with diffusion-based acc pseudotime.
    root_key: `str` or `None` (default: `'root_cells'`)
        Key (.uns, .obs) of root cell to be used.
        If not set, it obtains root cells from acc-inferred transition matrix.
...
def rank_dynamical_genes(data, n_genes=100, groupby=None, copy=False):
    ...
    groupby = (
        ...
        if "acc_clusters" in adata.obs.keys()
        else None
    )
