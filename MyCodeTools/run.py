...
from . import acc, acc_embedding, acc_graph
...
def run_all(
    ...
    acc(adata, mode=mode)
    print("Number of genes to be used:", adata.var.acc_genes.sum())

    acc_graph(adata)
    if basis is not None:
        acc_embedding(adata, basis=basis)
...
def convert_to_adata(vlm, basis=None):
    ...
    if hasattr(vlm, "q"):
        var["acc_offset"] = vlm.q
    if hasattr(vlm, "gammas"):
        var["acc_gamma"] = vlm.gammas
    if hasattr(vlm, "R2"):
        var["acc_r2"] = vlm.R2
    ...
    if hasattr(vlm, "corrcoef"):
        adata.uns["acc_graph"] = vlm.corrcoef

    if basis is not None:
        if hasattr(vlm, "ts"):
            adata.obsm[f"X_{basis}"] = vlm.ts
        if hasattr(vlm, "delta_embedding"):
            adata.obsm[f"acc_{basis}"] = vlm.delta_embedding
...
def convert_to_loom(adata, basis=None):
    ...
    class VelocytoLoom(velocyto.VelocytoLoom):
        def __init__(self, adata, basis=None):
            ...
            if "acc" in adata.layers.keys():
                self.acc = np.array(adata.layers["acc"].T, **kwargs)
            ...
            if isinstance(basis, str) and f"X_{basis}" in adata.obsm.keys():
                ...
                if f"acc_{basis}" in adata.obsm.keys():
                    self.delta_embedding = adata.obsm[f"acc_{basis}"]
        ...
        def acc_graph(
            ...
            if not hasattr(self, "ts"):
                ...
                # counterpart to scv.tl.acc_graph()
                ...
                # counterpart to scv.tl.acc_embedding()
                self.calculate_embedding_shift(
                    sigma_corr=sigma_corr, expression_scaling=expression_scaling
                )

        def acc_embedding(self, smooth=0.5, steps=(50, 50), n_neighbors=100):
        ...
        def run_all(
            ...
            self.acc_estimation(limit_gamma=limit_gamma, fit_offset=fit_offset)
            print(f"acc Estimation: {round(time() - timestamp, 2)}")
            timestamp = time()
            ...
            print(f"acc Graph: {round(time() - timestamp, 2)}")
