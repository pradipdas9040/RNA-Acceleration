...
from .rank_acc_genes import acc_clusters
from .utils import strings_to_categoricals
from .acc_graph import vals_to_csr
    ...
    def compute_transitions(self):
        ...
        if vkey not in self._adata.uns:
            raise ValueError(
                ...
                "with key 'acc_graph' - a sparse matrix from RNA acc."
            )
        if self._adata.uns[vkey].shape != (self._adata.n_obs, self._adata.n_obs):
            raise ValueError(
                f"The passed 'acc_graph' has shape {self._adata.uns[vkey].shape} "
...
def paga(
    ...
    """PAGA graph with acc-directed edges.
    ...
    use_time_prior : `str` or bool, optional (default: True)
        ...
        If True, 'acc_pseudotime' is used if available.
    ...
    elif groups == "acc_clusters" and "acc_clusters" not in adata.obs.keys():
        acc_clusters(adata)
    if use_time_prior and not isinstance(use_time_prior, str):
        use_time_prior = "acc_pseudotime"
        if use_time_prior not in adata.obs.keys():
            acc_pseudotime(adata, vkey=vkey, root_key=root_key, end_key=end_key)
