...
def acc_graph(
    ...
    r"""Computes acc graph based on cosine similarities.
    ...
    gene_subset: `list` of `str`, subset of adata.var_names or `None`(default: `None`)
        Subset of genes to compute acc graph on exclusively.
    ...
    if isinstance(basis, str):
        logg.warn(
            f"The acc graph is computed on {basis} embedding coordinates.\n"
    ...
    n_jobs = get_n_jobs(n_jobs=n_jobs)
    logg.info(
        f"computing acc graph (using {n_jobs}/{os.cpu_count()} cores)", r=True
    )
