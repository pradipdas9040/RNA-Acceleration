...
# TODO: Make equivalent to calling `scvelo.tools.acc(adata, mode="deterministic")`
# TODO: Save acc (residual) to `state_dict`
class SteadyStateModel(BaseInference):
    """Steady-state model for acc estimation.
    ...
    def _initialize_state_dict(self):
        self._state_dict = SteadyStateParams(
            ...
            acc_genes=np.zeros(self.n_vars, dtype=bool),
        )
    ...
    def fit(self) -> None:
        ...
        if self._highly_variable is not None:
            sd.acc_genes &= self._highly_variable

        if np.sum(sd.acc_genes) < 2:
            min_r2 = np.percentile(sd.r2, 80)
            sd.acc_genes = sd.r2 > min_r2
    ...
    def get_acc(self) -> None:
        """Get acc from steady-state model."""
...
class SecondOrderSteadyStateModel(SteadyStateModel):
    """Second-order steady-state model for acc estimation.
    ...
    def fit(self):
        ...
        super().fit()
        sd = self._state_dict
        idx = np.ones(sd.acc_genes.shape, dtype=bool)
        if np.any(sd.acc_genes):
            idx = sd.acc_genes
    ...
    def get_variance_acc(self):
        """Get variance of acc from steady-state model."""
