...
def acc_graph(
    ...
    r"""Computes acceleration graph based on cosine similarities.
    The cosine similarities are computed between accelerations and potential cell state
    ...
    vkey: `str` (default: `'acceleration'`)
        Name of acceleration estimates to be used.
    xkey: `str` (default: `'Ms'`)
    ...
    gene_subset: `list` of `str`, subset of adata.var_names or `None`(default: `None`)
        Subset of genes to compute acceleration graph on exclusively.
    ...
    Returns
    -------
    acceleration_graph: `.uns`
        sparse matrix with correlations of cell state transitions with accelerations
    """
    adata = data.copy() if copy else data
    verify_neighbors(adata)
    if vkey not in adata.layers.keys():
        os.access(adata, vkey=vkey)
    ...
    if isinstance(basis, str):
        logg.warn(
            f"The acceleration graph is computed on {basis} embedding coordinates.\n"
    ...
    n_jobs = get_n_jobs(n_jobs=n_jobs)
    logg.info(
        f"computing acceleration graph (using {n_jobs}/{os.cpu_count()} cores)", r=True
    )
